...
let memberList = [];
const totalMembers = bills.reduce((accumulator, bill) => {
  if (bill.member !== null) {
    if (!memberList.includes(bill.member.name)) {
      memberList.push(bill.member.name);
...
const billMembers = newBills.map((bill) => bill.member.name);
